...
static constexpr int DXCC_ENTITY_COUNT = sizeof(DXCC_ENTITIES) / sizeof(DXCC_ENTITIES[0]);

DXCCRepository::DXCCRepository()
    : m_db(GlobalDatabase::instance())
{
}

int DXCCRepository::getEntityCode(const QString& entityName) const {
    if (!m_cacheLoaded) {
        loadCache();
    }

    return m_nameToCodeCache.value(entityName.toUpper(), 0);
}

QString DXCCRepository::getEntityName(int entityCode) const {
    QSqlQuery query = m_db.execute(
        "SELECT entity_name FROM dxcc_entities WHERE entity_code = ?",
        {entityCode}
    );
...
bool DXCCRepository::initializeDXCCEntities() {
    ...
    QSqlQuery insertQuery = m_db.execute(
        "INSERT OR REPLACE INTO dxcc_entities (entity_code, entity_name) VALUES (?, ?)"
    );

    for (int i = 0; i < DXCC_ENTITY_COUNT; ++i) {
        insertQuery.bindValue(0, DXCC_ENTITIES[i].code);
        insertQuery.bindValue(1, QString(DXCC_ENTITIES[i].name));
    ...
    m_cacheLoaded = false;
    m_nameToCodeCache.clear();

    return true;
}

bool DXCCRepository::isPopulated() const {
    QSqlQuery query = m_db.execute("SELECT COUNT(*) FROM dxcc_entities");
...
void DXCCRepository::loadCache() const {
    m_nameToCodeCache.clear();

    QSqlQuery query = m_db.execute("SELECT entity_name, entity_code FROM dxcc_entities");

    while (query.next()) {
        QString name = query.value(0).toString().toUpper();
        int code = query.value(1).toInt();
        m_nameToCodeCache[name] = code;
    }
