#ifndef KPA1500UDPPOLLER_H
#define KPA1500UDPPOLLER_H
...
/**
 * @brief KPA1500 Amplifier UDP Poller
 *
 * Periodically polls an Elecraft KPA1500 amplifier over UDP using the ASCII
 * command set described in the KPA1500 Programming Reference.
 *
 * Protocol:
 ...
 *
 * Typical usage:
 * - Create in main thread, configure address and poll commands
 * - Move to a QThread and call start() when the thread starts
 * - Connect to signals like forwardPowerChanged(), swrChanged(), etc.
 *
 * Example:
 * @code
 * auto *poller = new KPA1500UdpPoller(this);
 * poller->setAmplifierAddress(QHostAddress("192.168.1.100"));
 * poller->setPollCommands({"^PWF;", "^PWI;", "^PWR;", "^SW;", "^TM;", "^VI;"});
 * connect(poller, &KPA1500UdpPoller::forwardPowerChanged,
 *         this, &MyClass::onPowerChanged);
 * poller->start();
 * @endcode
 */
class KPA1500UdpPoller : public QObject {
    Q_OBJECT

public:
    explicit KPA1500UdpPoller(QObject* parent = nullptr);
    ~KPA1500UdpPoller() override;

    /**
     * Set the amplifier's network address and port
     * @param addr IP address of the KPA1500
     * @param port UDP port (default: 1500)
     */
    void setAmplifierAddress(const QHostAddress& addr, quint16 port = 1500);

    /**
     * Set the polling interval in milliseconds
     * @param intervalMs Interval between poll cycles (default: 100ms)
     */
    void setPollIntervalMs(int intervalMs);

    /**
     * Set the list of commands to send each poll cycle
     * @param commands List of ASCII commands (e.g., {"^PWF;", "^SW;", "^TM;"})
     */
    void setPollCommands(const QStringList& commands);

signals:
    // Power and SWR monitoring
    void forwardPowerChanged(int watts);     // ^PWF - Forward power output
    void inputPowerChanged(int watts);       // ^PWI - Input power from radio
    void reflectedPowerChanged(int watts);   // ^PWR - Reflected power
    void swrChanged(double swr);             // ^SW - Standing wave ratio

    // Band and antenna switching
    ...
    // ATU (Antenna Tuning Unit) status
    ...
    // Combined status (forward/reflected/SWR in one message)
    void wsStatusChanged(int fwd, int ref, double swr); // ^WS
...
public slots:
    /**
     * Start polling (call after moving to QThread and starting thread)
     */
    void start();
