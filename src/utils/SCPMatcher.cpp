...
QStringList SCPMatcher::findMatches(const QString& partial) {
    LOG_DEBUG("SCPMatcher", QString("findMatches: m_enabled=%1, partial='%2', length=%3")
        .arg(m_enabled ? "true" : "false").arg(partial).arg(partial.length()));
    ...
    SCPRepository repo;
    QStringList matches = repo.findMatches(partial);
